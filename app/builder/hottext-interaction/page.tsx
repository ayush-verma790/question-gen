...
export default function ImageHottextBuilder() {
  ...
  const parseXMLToQuestion = (xmlString: string): Partial<QuestionData> => {
    try {
      ...
      const assessmentItem = xmlDoc.getElementsByTagName(
        "qti-assessment-item"
      )[0];
      if (!assessmentItem) {
      ...
      const correctResponseElement = xmlDoc.getElementsByTagName(
      ...
      for (let i = 0; i < hottextElements.length; i++) {
        ...
        if (img) {
          ...
          if (span && span.getAttribute("style")) {
            ...
            const borderRadiusMatch = styleAttr.match(
              /border-radius:\s*([^;]+)/
            );
      ...
      const feedbackElements =
        xmlDoc.getElementsByTagName("qti-modal-feedback");
      let correctFeedback = "";
      ...
      for (let i = 0; i < feedbackElements.length; i++) {
        ...
        const contentBody =
          feedback.getElementsByTagName("qti-content-body")[0];
    ...
    return blocks
      .map((block) => {
        ...
        switch (block.type) {
          ...
          case "video":
            return `<video controls style="${styleStr}"><source src="${
              block.content || ""
            }" /></video>`;
          case "audio":
            return `<audio controls style="${styleStr}"><source src="${
              block.content || ""
            }" /></audio>`;
          case "html":
  ...
  const getCombinedContent = (text: string, blocks: ContentBlock[]): string => {
    ...
    return (
      text +
      (blocksHTML ? `<div class="content-blocks">${blocksHTML}</div>` : "")
    );
  };
  ...
  const generateXML = () => {
    ...
    const correctValue =
      correctOptions.length > 0 ? correctOptions[0].identifier : "";
    ...
    const hottextElements = question.options
      .map((option) => {
        ...
        return `          <qti-hottext identifier="${
          option.identifier
        }"><span style="display: inline-block; ${styleStr}"><img src="${
          option.imageUrl
        }" alt="${option.altText}" width="${
          option.style?.width || "100px"
        }" height="${option.style?.height || "100px"}"/></span>
          </qti-hottext>`;
      })
      .join("\n");

    // Combine question text with content blocks
    const questionContent = getCombinedContent(
      question.questionText,
      question.questionBlocks
    );
    const correctFeedbackContent = getCombinedContent(
      question.correctFeedback,
      question.correctFeedbackBlocks
    );
    const incorrectFeedbackContent = getCombinedContent(
    ...
    <div style="${containerStyleStr}">
      ...
      ${
        question.mainImageUrl
          ? `<img src="${question.mainImageUrl}" alt="${
              question.mainImageAlt
            }" style="${mainImageStyleStr}" height="${
              question.mainImageHeight || "100px"
            }" width="${question.mainImageWidth || "100px"}" />`
          : ""
      }
  ...
  <qti-modal-feedback outcome-identifier="FEEDBACK" identifier="CORRECT" show-hide="show" title="Great Job!">
    ${
      correctFeedbackContent
        ? `<qti-content-body>${correctFeedbackContent}</qti-content-body>`
        : ""
    }
  </qti-modal-feedback>
        ...
        <div className="flex-1 min-w-0">
          <div className="mb-6">
            <h1 className="text-3xl font-bold text-gray-900 mb-2">
              Image Hottext Question Builder
            </h1>
            <p className="text-gray-600">
              Create interactive image-based hottext questions with selectable
              image options
            </p>
          </div>

          <div className="w-full space-2-6">
            {activeSection === "question" && (
              <div className="space-y-8">
                ...
                <Collapsible>
                  <CollapsibleTrigger asChild>
                    <Button
                      variant="outline"
                      className="w-full mb-4 bg-emerald-50 border-emerald-200 hover:bg-emerald-100 text-emerald-700"
                    >
                    ...
                    <Card className="shadow-lg border-0 bg-white/90 backdrop-blur-md rounded-2xl overflow-hidden mb-8">
                      <CardHeader className="bg-gradient-to-r from-emerald-50 to-teal-50 border-b border-emerald-100/50 pb-4">
                        ...
                        <p className="text-sm text-emerald-700 mt-2">
                          Paste your existing QTI XML to automatically populate
                          all form fields
                        </p>
                      </CardHeader>
              ...
              <Card className="shadow-lg border-0 bg-white/90 backdrop-blur-md rounded-2xl overflow-hidden">
                <CardHeader className="bg-gradient-to-r from-green-50 to-emerald-50 border-b border-green-100/50 pb-4">
                  ...
                  <p className="text-sm text-green-700 mt-2">
                    Configure your image-based answer choices with custom
                    styling
                  </p>
                </CardHeader>
                <CardContent className="space-y-6 p-6">
                  {question.options.map((option, index) => (
                    <Card
                      key={option.id}
                      className="border-2 border-gray-100 hover:border-gray-200 transition-colors rounded-xl overflow-hidden"
                    >
                      ...
                      <CardContent className="space-y-6 p-6">
                          ...
                          <div className="space-y-6">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                              <div>
                                ...
                                <Input
                                  value={question.mainImageWidth}
                                  onChange={(e) =>
                                    setQuestion((prev) => ({
                                      ...prev,
                                      mainImageWidth: e.target.value,
                                    }))
                                  }
                                  placeholder="400px"
                                  className="h-11 border-2 border-gray-200 focus:border-blue-400 rounded-xl"
                              ...
                              <div>
                                ...
                                <Input
                                  value={question.mainImageHeight}
                                  onChange={(e) =>
                                    setQuestion((prev) => ({
                                      ...prev,
                                      mainImageHeight: e.target.value,
                                    }))
                                  }
                                  placeholder="300px"
                                  className="h-11 border-2 border-gray-200 focus:border-blue-400 rounded-xl"
                        ...
                        <Collapsible
                          ...
                          className="rounded-xl border border-blue-100 bg-gradient-to-r from-blue-50/50 to-indigo-50/50 p-4"
                        >
                ...
                <CardContent className="space-y-6">
                  <div className="border-t pt-6">
                    <Label className="text-sm font-medium">
                      Incorrect Answer Feedback (Rich HTML Editor)
                    </Label>
                    <RichTextEditor
            ...
            {activeSection === "preview" && (
              <div className="space-y-6">
                  ...
                  <CardContent>
                    <div className="bg-white p-4 rounded-lg border min-h-48">
                      <div
                        style={{
                          fontFamily:
                            question.containerStyle?.fontFamily || "Canva Sans",
                          fontSize: question.containerStyle?.fontSize || "22px",
                          backgroundColor:
                            question.containerStyle?.backgroundColor ||
                            "#ffffff",
                          padding: question.containerStyle?.padding || "20px",
                          borderRadius:
                            question.containerStyle?.borderRadius || "8px",
                          boxShadow:
                            question.containerStyle?.boxShadow ||
                      ...
                      >
                        ...
                        {question.mainImageUrl && (
                          <img
                            ...
                            style={{
                              width: question.mainImageStyle?.width
                                ? `${question.mainImageStyle.width}px`
                                : "auto",
                              height: question.mainImageStyle?.height
                                ? `${question.mainImageStyle.height}px`
                                : "auto",
                              margin:
                                question.mainImageStyle?.margin || "10px 0",
                            }}
                          />
                        )}
                          ...
                          {question.options.map((option) => (
                            <div
                              ...
                              className={`hover:opacity-75 ${
                                option.isCorrect ? "ring-2 ring-green-400" : ""
                              }`}
                              title={
                                option.isCorrect ? "Correct Answer" : "Option"
                              }
                            >
                              {option.imageUrl ? (
                                <img
                ...
                <PreviewRenderer
                  ...
                  questionType="hottext-interaction"
                />
              </div>
            )}
          </div>
